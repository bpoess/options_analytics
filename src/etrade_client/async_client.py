...
class AsyncETradeClient:
    """Async E*Trade API client with integrated OAuth1 session management."""
    ...
    async def _load_cached_token(self) -> dict | None:
        """Load token from disk if it exists."""
        if not self._token_path.exists():
            return None
        async with aiofiles.open(self._token_path) as f:
            content = await f.read()
        self._token_mtime = self._token_path.stat().st_mtime
        return json.loads(content)

    async def _save_token(self, token: dict) -> None:
    ...
    async def _renew_session(self, token: dict) -> bool:
        ...
        try:
            ...
            await self._http.get(signed_url, headers=signed_headers)
            return True
        except Exception as e:
    ...
    async def get_authorization_url(self) -> str:
        ...
        response = await self._http.post(signed_url, headers=signed_headers)
    ...
    async def complete_authorization(self, verification_code: str) -> None:
    ...
    async def fetch_order_details(self, order: dict) -> dict | None:
        """Fetch details for a single order and return raw response."""
        headers = {"Accept": "application/json"}
        details_url = order["details"]

        response = await self._get(details_url, headers=headers)
        response.raise_for_status()
    ...
    async def fetch_option_chains(
        ...
        """Fetch list of option chains for a specific underlying instrument."""
        url = f"{self._base_url}/v1/market/optionchains"
        headers = {"Accept": "application/json"}
        params = {
            "symbol": symbol,
        ...
        response = await self._get(url, headers=headers, params=params)
        response.raise_for_status()
        return response.json().get("OptionChainResponse")

    async def fetch_option_expire_dates(
        self, symbol: str, expiry_type: str | None = None, num_retries: int = 5
    ) -> dict | None:
        """Return a list of valid expiry dates for options of the given symbol."""
        url = f"{self._base_url}/v1/market/optionexpiredate"
        headers = {"Accept": "application/json"}
        params = {
            "symbol": symbol,
            "expiry_type": str,
        ...
        if response.status_code == 302:
            # Workaround for a flaky etrade API, it sometimes returns 302
            if num_retries > 0:
                return await self.fetch_option_expire_dates(
                    symbol, expiry_type, num_retries - 1
                )

        response.raise_for_status()
        return response.json().get("OptionExpireDateResponse")
